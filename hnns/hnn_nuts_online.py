...
#         monitor = np.log(logu) + joint.numpy()
...
#         print(f"Recursion: Implicitly build the height {j - 1} left and right subtrees.")
...
def hnn_nuts_sampling(args, control_group):
    ...
    for ii in range(int(args.input_dim / 2), args.input_dim):
        y0[ii] = norm(loc=0, scale=1).rvs()
    num_chains = M + Madapt
    initial_q = tf.zeros([num_chains, input_dim // 2], dtype=tf.float32)  # 位置 q
    initial_p = tf.random.normal([num_chains, input_dim // 2], dtype=tf.float32)  # 动量 p
    ...
    total_gradient_evaluations = tf.reduce_sum(total_leapfrogs).numpy()
...
#     filename = os.path.join(save_dir, f"hnn_nuts_{args.dist_name}_{args.input_dim}.pkl")
#     # 保存为 Pickle 文件
#     with open(filename, "wb") as file:
#         pickle.dump(result, file)
...
if __name__ == "__main__":
    ...
    hnn_nuts_sampling(args, control_group)
