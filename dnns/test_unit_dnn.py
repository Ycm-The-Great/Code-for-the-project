...
from dnn_hmc import  get_control_group_hmc, dnn_sampling
from dnn_lmc import  get_control_group_lmc, dnn_lmc_sampling
from dnn_nuts_online import get_model, get_args_NUTs, dnn_nuts_sampling
import os
...
class TestIntegration(unittest.TestCase):
    ...
    def test_01_dnn_hmc(self):
        """
        测试 dnn_hmc.py 的逻辑
        """
        print("Unitest: dnn_hmc")
        # print(self.args)

        dnn_sampling(self.args, self.hmc_args)  # 调用 hnn_hmc 的采样逻辑

    def test_02_dnn_lmc(self):
        """
        测试 dnn_lmc.py 的逻辑
        """
        print("Unitest: dnn_lmc")
        #print(self.args)
        dnn_lmc_sampling(self.args,self.lmc_args)  # 调用 hnn_lmc 的采样逻辑

    def test_03_dnn_nuts_online(self):
        """
        测试 dnn_nuts_online.py 的逻辑
        """
        print("Unitest: dnn_nuts_online")
        #print(self.args)
        hnn_model = get_model(self.args, baseline=True)
        dnn_nuts_sampling(self.args, self.nuts_args)  # 调用 hnn_nuts_online 的采样逻辑
